...
class CFR:
    ...
    def cfr(self, game, player, pi, po):
        # Si l'état du jeu est terminal, retourner la récompense positive ou négative
        if self.isTerminal():
            if game.is_dudo()
            return game.getReward(player)
    ...
    def gameReset(self):
        return NewPerudo(random.randint(1, 5), random.randint(1, 5)) 
